  ...
  <div class="">
    ...
    <form @submit.prevent="crearCliente">
      ...
      <div class="mb-4">
        ...
        <input v-model="nota" type="text" id="nota" required
               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>

      <div class="mb-4">
        <label for="fechaNacimiento" class="block text-sm font-medium text-gray-700 mb-1">Fecha De Cumpleaños *</label>
      ...
      <div class="mb-6">
        ...
        <input v-model="direccion.referencia" type="text" id="direccion_referencia" required
               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
      ...
      <div class="mb-6">
          ...
          <template #item="{ element, index }">
            <div class="mb-4 p-2 bg-gray-100 rounded-md">
              <div class="flex items-center space-x-2">
                ...
                <button type="button" @click="toggleContactType(index)"
                ...
                <template v-if="element.tipo === 'telefono-movil'">
                  <div class="flex items-center space-x-2">
                    <button type="button" @click="indexContactoSeleccionado = index; showModal = true;" class="w-20 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center justify-between">
                      <span>{{ element.codigoTelefono }}</span>
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                      </svg>
                    </button>
                    <input v-model="element.telefono" type="tel" placeholder="123456789" class="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                </template>
                <template v-else>
                  <input v-model="element.correo" type="email" placeholder="email@example.com" class="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </template>
                <button type="button" @click="removeContact(index)" class="text-red-500 hover:text-red-600">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <button type="button" @click="addContact" class="text-blue-500 hover:text-blue-600">
